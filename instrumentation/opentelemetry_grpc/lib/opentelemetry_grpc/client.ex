defmodule OpentelemetryGrpc.Client do
  ...
  defp record_client_span_result({:error, %GRPC.RPCError{status: status, message: message}}) do
    ...
    otel_status = grpc_status_to_otel_status(status)
    status_name = GRPC.Status.code_name(status)

    Tracer.set_status(otel_status, message || "gRPC error: #{status_name}")
  end
  ...
  defp grpc_status_to_otel_status(0), do: :ok
  defp grpc_status_to_otel_status(_), do: :error
end
