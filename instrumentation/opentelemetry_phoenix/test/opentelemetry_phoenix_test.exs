defmodule OpentelemetryPhoenixTest do
  ...
  test "records request headers as configured for Phoenix web requests" do
    OpentelemetryPhoenix.setup(headers_attrs: ["foo", "bar"])
    ...
    meta = %{
      ...
      | conn: %{
          ...
          | req_headers: [
              {"foo", "foo-1"},
              {"foo", "foo-2"},
              {"bar", "bar-1"} | meta.conn.req_headers
            ]
        }
    }
    ...
    assert %{
             ...
             "phoenix.action": :user,
             "phoenix.plug": Elixir.MyStoreWeb.PageController,
             "http.request.header.foo": ["foo-2", "foo-1"],
