  ...
  ContainerIDAttributes =
    case fetch_container_id_attributes(Config) of
      ...
      {error, ContainerIDAttributesError} ->
        erlang:error(ContainerIDAttributesError),
        []
    end,
  ...
  MetadataV4Attributes =
    case fetch_metadata_v4_attributes() of
      ...
      {error, MetadataV4AttributesError} ->
        erlang:error(MetadataV4AttributesError),
        []
    end,
  ...
  case file:read_file(Path) of
      {ok, Data} ->
          ...
          case ContainerId of
              ...
              ContainerIdValue -> {ok, [{?CONTAINER_ID, ContainerIdValue}]}
          end;
      {error, Error} ->
        ...
        {ok, TaskMetadata} ->
          ...
          Attributes =
            [
              ...
              error -> []
            end ++ case (maps:is_key(<<"LogOptions">>, ContainerMetadata) andalso maps:get(<<"LogDriver">>, ContainerMetadata) =:= "awslogs") of
              true ->
